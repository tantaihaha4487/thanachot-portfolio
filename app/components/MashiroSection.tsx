"use client";

import React from "react";
import { motion } from "framer-motion";
import Image from "next/image";
import TiltCard from "./TiltCard";

const traits = [
  { key: "vtuber", label: "AI VTuber Initiative", desc: "Aimed to be the first interactive Thai AI VTuber" },
  { key: "persona", label: "Playful & Mischievous", desc: "Silly, friendly, and uniquely expressive personality" },
  { key: "lore", label: "Divine Feline", desc: "1000-year-old cat god with white purple hair" },
  { key: "status", label: "Currently on Hiatus", desc: "Development paused due to resource constraints" },
];

function FadeUp({ children, delay = 0 }: { children: React.ReactNode; delay?: number }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 28 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: false, margin: "-100px" }}
      transition={{ duration: 0.6, delay, ease: "easeOut" }}
    >
      {children}
    </motion.div>
  );
}

export default function MashiroSection() {
  return (
    <section id="mashiro" className="relative section-pad overflow-hidden">
      {/* Background accent */}
      <div
        className="absolute top-1/2 -translate-y-1/2 right-0 w-96 h-96 opacity-8 blur-3xl pointer-events-none"
        style={{ background: "radial-gradient(circle, rgba(167,139,250,0.3) 0%, transparent 70%)" }}
      />

      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-12">
        {/* Section header */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: false, margin: "-80px" }}
          transition={{ duration: 0.6 }}
          className="mb-14"
        >
          <div className="flex items-center gap-3 mb-3">
            <span className="mono-label syn-comment">// section_02</span>
            <span className="w-12 h-px opacity-20" style={{ background: "#C084FC" }} />
          </div>
          <h2
            className="text-3xl sm:text-4xl lg:text-5xl font-bold"
            style={{ fontFamily: "var(--font-geist-sans)" }}
          >
            Project{" "}
            <span style={{ color: "#C084FC" }}>Mashiro</span>
          </h2>
        </motion.div>

        {/* Main card */}
        <TiltCard
          className="glass-purple rounded-2xl overflow-hidden relative group cursor-pointer transition-all duration-300 hover:scale-[1.01]"
          maxTilt={6}
          style={{ boxShadow: "0 4px 80px rgba(0,0,0,0.4)" }}
        >
          {/* Corner wireframe accents */}
          <div
            className="absolute top-0 left-0 w-16 h-16 pointer-events-none opacity-40"
            style={{
              borderTop: "1px solid #C084FC",
              borderLeft: "1px solid #C084FC",
            }}
          />
          <div
            className="absolute bottom-0 right-0 w-16 h-16 pointer-events-none opacity-40"
            style={{
              borderBottom: "1px solid #C084FC",
              borderRight: "1px solid #C084FC",
            }}
          />

          <div className="flex flex-col lg:flex-row">
            {/* ── LEFT: Character image ── */}
            <div
              className="flex-shrink-0 w-full lg:w-72 xl:w-80 h-96 sm:h-80 lg:h-auto lg:min-h-[420px] flex items-end justify-center relative overflow-hidden border-b lg:border-b-0 lg:border-r"
              style={{
                background: "linear-gradient(180deg, rgba(40,44,51,0.2) 0%, rgba(192,132,252,0.06) 100%)",
                borderColor: "rgba(192,132,252,0.12)",
              }}
            >
              {/* Dot grid overlay */}
              <div className="absolute inset-0 dot-grid opacity-30" />

              <div className="relative z-10 w-full h-full">
                <Image
                  src="/mashiro.png"
                  alt="Mashiro"
                  fill
                  className="object-cover object-top"
                />
              </div>

              {/* Character name badge */}
              <div
                className="absolute bottom-6 left-1/2 -translate-x-1/2 rounded-full px-4 py-2 z-20"
                style={{
                  background: "rgba(30, 34, 42, 0.95)",
                  border: "1px solid rgba(192,132,252,0.3)",
                  boxShadow: "0 4px 20px rgba(0,0,0,0.4)",
                }}
              >
                <span
                  className="mono-label"
                  style={{ color: "#C084FC", letterSpacing: "0.09em" }}
                >
                  まっしろ · Mashiro
                </span>
              </div>
            </div>

            {/* ── RIGHT: Description ── */}
            <div className="flex-1 p-6 sm:p-8 lg:p-12 flex flex-col gap-6 sm:gap-8">
              {/* Status & tags */}
              <FadeUp delay={0}>
                <div className="flex flex-wrap gap-2">
                  {["AI VTuber", "LLM Integration", "Interactive Avatar", "On Hiatus"].map((tag) => (
                    <span
                      key={tag}
                      className="mono-label px-3 py-1 rounded-full"
                      style={{
                        background: "rgba(192,132,252,0.08)",
                        border: "1px solid rgba(192,132,252,0.2)",
                        color: "#C084FC",
                      }}
                    >
                      {tag}
                    </span>
                  ))}
                </div>
              </FadeUp>

              {/* Description */}
              <FadeUp delay={0.1}>
                <div className="flex flex-col gap-4">
                  <p
                    className="text-base leading-relaxed"
                    style={{ color: "#C4CDDA" }}
                  >
                    Mashiro is an ambitious project aiming to create the{" "}
                    <span style={{ color: "#C084FC" }}>first Thai AI VTuber</span>{" "}
                    capable of real-time viewer interaction. Designed as a 1000-year-old
                    cat god (まっしろ), she embodies a playful, slightly mischievous, yet friendly persona.
                  </p>
                  <p
                    className="text-sm leading-relaxed"
                    style={{ color: "#8892A4" }}
                  >
                    While the project successfully established her core personality—complete with
                    specific conversational nuances—active development is currently on hiatus
                    as I explore new inspirations and manage resource constraints.
                  </p>
                </div>
              </FadeUp>

              {/* Traits grid */}
              <FadeUp delay={0.2}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {traits.map((t) => (
                    <div
                      key={t.key}
                      className="p-4 rounded-xl transition-colors duration-200 hover:bg-white/5"
                      style={{
                        background: "rgba(255,255,255,0.02)",
                        border: "1px solid rgba(255,255,255,0.06)",
                      }}
                    >
                      <div
                        className="mono-label mb-1"
                        style={{ color: "#C084FC" }}
                      >
                        → {t.label}
                      </div>
                      <div
                        className="text-xs leading-relaxed"
                        style={{ color: "#6B7280" }}
                      >
                        {t.desc}
                      </div>
                    </div>
                  ))}
                </div>
              </FadeUp>

              {/* Philosophy quote */}
              <FadeUp delay={0.3}>
                <div
                  className="relative pl-4 py-1"
                  style={{ borderLeft: "2px solid rgba(192,132,252,0.4)" }}
                >
                  <p
                    className="text-sm italic leading-relaxed"
                    style={{ color: "#8892A4" }}
                  >
                    <span className="syn-comment">// system_prompt</span>
                    <br />
                    &quot;Always respond as Mashiro, a 1000-year-old cat girl. Keep responses light,{" "}
                    <span style={{ color: "#E2E8F0" }}>playful, and uniquely expressive.</span>&quot;
                  </p>
                </div>
              </FadeUp>
            </div>
          </div>
        </TiltCard>
      </div>
    </section>
  );
}
